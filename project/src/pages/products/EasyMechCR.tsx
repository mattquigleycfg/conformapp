...
import { calculateEasyMechCR, type EasyMechCRInputs, type EasyMechCRResults } from '../../services/calculations/easyMechCRCalculator';

const FLOORING_TYPES = ['Mesh', 'Solid', 'Perforated'];
const LOAD_RATINGS = [2.5, 5.0];
const LOCATIONS = ['NSW', 'VIC', 'QLD', 'SA', 'WA', 'TAS', 'NT', 'ACT'];

const defaultInputs: EasyMechCRInputs = {
  width: 8400,
  length: 8400,
  flooring: 'Mesh',
  loadRating: 2.5,
...
const EasyMechCR: React.FC = () => {
  const navigate = useNavigate();
  const [inputs, setInputs] = useState<EasyMechCRInputs>(defaultInputs);
  const [results, setResults] = useState<EasyMechCRResults | null>(null);

  useEffect(() => {
    const newResults = calculateEasyMechCR(inputs);
    setResults(newResults);
  }, [inputs]);

  const handleInputChange = (name: keyof EasyMechCRInputs, value: string | number | boolean) => {
    setInputs(prev => ({
      ...prev,
      [name]: value
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          <h2 className="text-xl font-medium text-gray-800">EasyMech CR Parameters</h2>
